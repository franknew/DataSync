...
namespace Chainway.Library.SimpleMapper
{
    public class Common
    {
        ...
        public static string GetTableName(string key, string owner, Type t, TableConfig config, Dictionary<string, object> data)
        {
        ...
        public static SqlModel BuildWhere(List<WhereClause> wheres, ISQLConvert converter)
        {
        ...
        public static List<Parameter> MergeParameter(List<Parameter> left, List<Parameter> right)
        {
            if (right == null || right.Count == 0) return left;
            if (left == null) left = new List<Parameter>();
            right.ForEach(t =>
            {
                if (!left.Exists(p => p.Name.Equals(t.Name))) left.Add(t);
            });
            return left;
        }

        public static string ReplaceParameter(string sql, List<Parameter> parameters)
        {
            if (parameters == null) return sql;
            //parameters.ForEach(t =>
            //{
            //    switch (t.Type.Name.ToLower())
            //    {
            //        case "int32":
            //        case "int64":
            //        case "decimal":
            ...
            //        case "long":
            //        case "double":
            //            sql = sql.Replace(t.Name, string.Format("{0}", t.Value));
            //            break;
            //        case "datetime":
            //            sql = sql.Replace(t.Name, string.Format("'{0}'", Convert.ToDateTime(t.Value).ToString("yyyy-MM-dd HH:mm:ss")));
            //            break;
            //        default:
            //            sql = sql.Replace(t.Name, string.Format("'{0}'", t.Value));
            //            break;
        ...
        public static Type GetType(string toType, Type sourceType, object value)
        {
            ...
            else if (value != null) if (value.IsDate()) t = typeof(DateTime);
